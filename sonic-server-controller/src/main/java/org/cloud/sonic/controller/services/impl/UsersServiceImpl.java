...
import org.cloud.sonic.common.tools.RSAUtils;
import org.cloud.sonic.controller.mapper.MtUserMapper;
...
//                save(users);
//                System.out.println("ccc:"+users.toString());
                usersMapper.insertcom(users);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private boolean loginLocal(Users user, String password) {
        boolean flag = true;
        if (!RSAUtils.sign( password, user.getPassword())) {
            flag = false;
        }
        return flag;
    }
    ...
    public String login(UserInfo userInfo) {
        ...
        String token = null;
        boolean result = loginLocal(users, userInfo.getPassword());
        if (result){
            token = jwtTokenTool.getToken(userInfo.getUserName());
        }else {
            MtUser mtUser = mtUserMapper.selectById(users.getId());
            Users users1 = new Users(mtUser.getId(),mtUser.getPassword(),null,mtUser.getUsername(),UserLoginType.LOCAL);
            int insertcom = usersMapper.insertcom(users1);
            if (insertcom>0){
                result = loginLocal(users1, userInfo.getPassword());
            }
            if (result){
                token = jwtTokenTool.getToken(userInfo.getUserName());
            }
    ...
    @Override
    public int usersMoving() {
        List<MtUser> mtUsers = mtUserMapper.selectList(null);
        return usersMapper.usersMoving(mtUsers);
    }
}
